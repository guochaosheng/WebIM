...
var userId = getCookie(cookie_key);
var name = 	getCookie(userName);
var face = getCookie(userFace);
...
var config = {
    ...
    right: -232,
    api: {
        friend: 'user/loadFriends.do?userId=' + userId, //好友列表接口
        group: 'user/loadGroups.do?userId=' + userId, //群组列表接口 
        chatlog: 'chatlog.json', //聊天记录接口
        groups: 'user/loadUsers.do?groupId=', //群组成员接口
        sendurl: 'message/sendMessage.do', //发送消息接口
        receiveurl: 'message/receiveMessage.do?userId=' + userId //接口消息接口
    },
    user: { //当前用户信息
        name: name,
        face: face
...
xxim.popchat = function(param){
    ...
    log.success = function(layero){
        ...
        xxim.chatbox.find('.layim_close').on('click', function(){
            ...
            config.chating = {};
            config.chatings = 0;
            currentChat = null; //聊天窗关闭时当前聊天窗口重置为空
        });
...
xxim.tabchat = function(param){  
    ...
    //设置当前聊天窗口，并从messageMap取出保存的旧数据
    currentChat = param.id; // 当前user
    var arr = messageMap.get(currentChat);
    if (arr != null && arr[0].dataType == 0){ //user和group的ID不能相同，根据id取
	    for(var i=0; i<arr.length; i++) {
	    	var log = {};
			//此处皆为模拟
            var keys = xxim.nowchat.type + currentChat;
            log.imarea = xxim.chatbox.find('#layim_area'+ keys);
            log.imarea.append(xxim.chatemplate({
                time: new Date(arr[i].date).pattern("yyyy年MM月dd日 HH:mm:ss"),
	    ...
	    messageMap.remove(currentChat);
    } else if (groupMap.get(currentChat) != null){ //判断群成员信息是否已加载，未加载时跳过
    	xxim.grouplog();
    }
};
...
xxim.grouplog = function() {
    ...
    if (arr != null) {
		for(var i=0; i<arr.length; i++) {
	        ...
	        log.imarea.append(xxim.chatemplate({
	            time: new Date(arr[i].date).pattern("yyyy年MM月dd日 HH:mm:ss"),
	            name: groupMap.get(currentChat).get(arr[i].fromUserId).name,
	            face: groupMap.get(currentChat).get(arr[i].fromUserId).face,
	            content: arr[i].content
	        }));
	        log.imarea.scrollTop(log.imarea[0].scrollHeight);
	    }
...
xxim.getGroups = function(param){
    ...
    config.json(config.api.groups + currentChat, {}, function(datas){
        if(datas.status === 1){
        	// 将加载的群成员对象(信息) 如姓名、头像等存入Map
        	var group = new Map();
        	for(var i=0; i<datas.data.length; i++) {
        		group.put(datas.data[i].id, datas.data[i]);
        	}
        	groupMap.put(currentChat, group); // 将当前窗口加载的群成员对象Map存到全局的Map中
...
//聊天模版
xxim.chatemplate = function(param, type) {
...
//接收消息的递归函数
xxim.receive = function() {
	config.json(config.api.receiveurl, {}, function(result) {
		if (result.status==1) {
			//根据fromUserId放到不同数组中 var arr = [], arr.push(),在存于map中
			for(var i=0; i<result.data.length; i++) {
				var key = null;
				var name = null;
				var face = null;
				// Key决定了消息显示在哪个用户聊天窗，如果消息类型是单个用户，哪个用户发送，就在哪个用户聊天窗显示Key为来源对象，群聊的，Key为发送给的对象
				if (result.data[i].dataType == 0) {
					key = result.data[i].fromUserId;
				} else {
					key = result.data[i].toUserId;
				}
				//如果当前聊天创建对象是消息发送对象，则立即显示消息，否则把它们存到messageMap中
				if (key == currentChat) {
					var log = {};
					//此处皆为模拟
		            ...
		            log.imarea = xxim.chatbox.find('#layim_area'+ keys);
		            log.imarea.append(xxim.chatemplate({
	                    time: new Date(result.data[i].date).pattern("yyyy年MM月dd日 HH:mm:ss"),
	                    name: result.data[i].dataType == 0?xxim.nowchat.name:groupMap.get(currentChat).get(result.data[i].fromUserId).name,
	                    face: result.data[i].dataType == 0?xxim.nowchat.face:groupMap.get(currentChat).get(result.data[i].fromUserId).face,
	                    content: result.data[i].content
	                }));
			        log.imarea.scrollTop(log.imarea[0].scrollHeight);
				} else {
					// 将接收的消息缓存到messageMap中
					if (messageMap.containsKey(key)) {
						var arr = messageMap.get(key);
						arr.push(result.data[i]);
					} else {
						var arr = [];
						arr.push(result.data[i]);
						messageMap.put(key, arr);
					}
				}
			}
...
//            _: +new Date
//        };
    	var datatype = $("#layim_groups").css("display") == "block"? 1:0;
    	var data = {
              content: node.imwrite.val().trim(),
              fromUserId: userId,
              toUserId: xxim.nowchat.id,
              dataType: datatype,
              date: +new Date
            };
...
//            };
            ...
            log.imarea.append(xxim.chatemplate({
                time: new Date(data.date).pattern("yyyy年MM月dd日 HH:mm:ss"),
            ...
            config.json(config.api.sendurl, JSON.stringify(data), function(datas){
            });            
        }
    };
...
//搜索加友
xxim.searchpal = function() {
	$("#xxim_mymsg").on("click",function(){
...
xxim.view = (function(){
    var xximNode = xxim.layimNode = $('<div id="xximmm" class="xxim_main">'
            ...
            +'<li class="xxim_online" id="xxim_online">'
                ...
                +'</div>'
            +'</li>'
            +'<li class="xxim_mymsg" id="xxim_mymsg" title="搜索"><i></i><a "href='+ config.msgurl +'" target="_blank"></a></li>'
